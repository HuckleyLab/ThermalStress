...
ts= array(NA, dim= c(length(years), nrow(tol.gt), 365,4) )
...
for(spec.k in 1:nrow(tol.gt)){
  ...
  ts[year.k,spec.k,,1]= tol.gt[spec.k,'Tmax']-tmax.k
  ts[year.k,spec.k,,2]= tol.gt[spec.k,'topt']-tmax.k 
  
  #metabolic scaled thermal stress
  inds= which(tmax.k > tol.gt[spec.k,'topt'])
  
  if(length(inds)>0) ts[year.k,spec.k,inds,3]= tmax.k[inds]- tol.gt[spec.k,'topt']  
...
plot.TSM90p= ggplot(tol.ts, aes(x=abs(lat_max),y=TSM90p) ) +geom_point()+facet_wrap(~habitat) +geom_smooth(method='loess',se=TRUE)
#lowest 7 and 14 day average 
plot.low7d= ggplot(tol.ts, aes(x=abs(lat_max),y=low7d) ) +geom_point()+facet_wrap(~habitat) +geom_smooth(method='loess',se=TRUE)
plot.low14d= ggplot(tol.ts, aes(x=abs(lat_max),y=low14d) ) +geom_point()+facet_wrap(~habitat) +geom_smooth(method='loess',se=TRUE)
#count of days <5
plot.count5= ggplot(tol.ts, aes(x=abs(lat_max),y=count5) ) +geom_point()+facet_wrap(~habitat) +geom_smooth(method='loess',se=TRUE)
...
plot.sumI= ggplot(tol.ts, aes(x=abs(lat_max),y=log(sumI)) ) +geom_point()+facet_wrap(~habitat) +geom_smooth(method='loess',se=TRUE)
plot.countI= ggplot(tol.ts, aes(x=abs(lat_max),y=log(countI)) ) +geom_point()+facet_wrap(~habitat) +geom_smooth(method='loess',se=TRUE)
plot.meanI= ggplot(tol.ts, aes(x=abs(lat_max),y=log(meanI)) ) +geom_point()+facet_wrap(~habitat) +geom_smooth(method='loess',se=TRUE)
