...
pdf("Fig1a_TPCs.pdf", height = 10, width = 4)
par(mfrow=c(5,1), cex=1.1, lwd=1, mar=c(3,3,0,0), mgp=c(1.3, 0.5, 0), oma=c(0,2,0,0), bty="l", cex.lab=1.2)
...
if(taxa %in% c(1,2,4)) plot(temps, tpc.plot(temps, Topt=tpc.sub[1,"Topt"] , CTmin=tpc.sub[1,"CTmin"], CTmax=tpc.sub[1,"CTmax"]), type="l", xlab="",ylab="", main="") #taxas[taxa]
...
if(taxa==5) plot(temps, tpc.plot(temps, Topt=tpc.sub[1,"Topt"] , CTmin=tpc.sub[1,"CTmin"], CTmax=tpc.sub[1,"CTmax"]), type="l", xlab="temperature (C)",ylab="", main="")
...
fig1b= ggplot(tpc2) + aes(x=Topt, y = breadth, color=habitat)+geom_point()+facet_grid(taxa~section)+theme(legend.position="bottom")
...
par(mfrow=c(5,4), cex=1.1, lwd=1, mar=c(3,3,1,0), mgp=c(1.3, 0.5, 0), oma=c(0,0,0,0), bty="l", cex.lab=1.2)
taxas= c("flies","insects","lizards","lizards_Tp","plankton")
