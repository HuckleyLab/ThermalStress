...
tpc0.2= as.data.frame(rbind( c(20, -5, 35),c(25, 0, 40), c(30, 4, 45)) )
tpc0.2$scen="shift in TPC"
tpc0= rbind(tpc0, tpc0.2)

out=t(apply(tpc0[,1:3], MARGIN=1, FUN=tpc.mat))
tpc0$asym= c(0.0, 0.25, 0.5, 35, 40, 45)
...
#to long format
tpc.l<- tpc.pred %>%
  gather("temperature", "performance", 6:ncol(tpc.pred))
...
fig0a= ggplot(tpc.l)+aes(x=temperature, y = performance)+ facet_wrap(~scen)+
  geom_line(aes(color=asym, group=asym))+
  ...
  geom_segment(data=tpc0, aes(x = 35, y = 1, xend = 35, yend = tpc.plot(35,tpc0[2,1],tpc0[2,2],tpc0[2,3]) ), colour = "blue", size=2)+ #draw performance detriment
...
#plot
tpc1= as.data.frame(cbind(tpc.p[,2,1], tsm[,1], 1:length(asyms) ))
tpc1$scen="shift in asymmetry"
tpc1$var="TSM"

tpc2= as.data.frame(cbind(tpc.p[,2,2], tsm[,2], 1:length(asyms) ))
tpc2$scen="shift in TPC"
tpc2$var="TSM"

tpc3= as.data.frame(cbind(tpc.p[,2,1], pd[,1], 1:length(asyms) ))
tpc3$scen="shift in asymmetry"
tpc3$var="pd"

tpc4= as.data.frame(cbind(tpc.p[,2,2], pd[,2], 1:length(asyms) ))
...
#plot
fig0b= ggplot(tpc.pl)+aes(x=Topt, y = metric, color=asyms[k])+geom_point()+facet_grid(var~scen, scales="free_y")+
